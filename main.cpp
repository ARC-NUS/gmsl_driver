...
int main(int argc, const char **argv)
{
	...
	ProgramArguments arguments(
			...
			ProgramArguments::Option_t("topic", "camera/image"),
...
void runNvMedia_pipeline(WindowBase *window, dwRendererHandle_t renderer, dwContextHandle_t sdk, std::vector<Camera> &cameras)
{
	...
	for (int i = 0; i < cameras.size(); i++) {  // for each port
		...
		for (int neighbor = 0; neighbor < cameras[i].numSiblings; neighbor++) {
			...
			pub[camIn] = it.advertise(topicexit, 6);
			++camIn;
		}
	}
	...
	else
	{
		...
		while (g_run && ros::ok()) {
			...
			for (int i = 0; i < cameras.size(); i++) {
				...
				for (int camIdx = camIdxOld; camIdx < camIdxOld + camera.numSiblings; camIdx++){
					...
					if (frame && camera.rgbaImagePool.size() > 0) {
						...
						else 
						{
							...
							if (status == DW_SUCCESS && d_frame) 
							{
								...
								int height =  d_rgb.prop.height;
								int width = d_rgb.prop.width;
								...
								// cv_connectors[camIdx]->pub.publish(img_msg);
								pub[camIdx].publish(img_msg);
